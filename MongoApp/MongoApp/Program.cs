...
namespace MongoApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var userRec = db.LoadRecordById<UserDetails>("horoscope", new Guid("d7cdef35-ef7a-46d4-bf24-0abb7d02f1cd"));
            userRec.BrideName = "Sobha";
            userRec.GroomName = "Nuthan";
            db.UpdateRecord<UserDetails>("horoscope", userRec.Id, userRec);

            Console.WriteLine($"Horoscope status: {userRec?.horoscope?.MatchStatus} and reason: {userRec?.horoscope?.Reason}");

            // db.DeleteRecord<UserDetails>("horoscope", userRec.Id);
